/*
    * DDL(DATA DEFINITION LANGUAGE) 데이터 정의 언어
    ...
    오라클에서 제공하는 객체를 새롭게 만들고, 구조를 변경하고, 구조를 삭제할 수 있는 명령문.
...
/*
    ...
    - 문자 (CHAR(크기)/VARCHAR2(크기) :  크기 BYTE수로 지정, 글자길이 CHAR로 지정. 
                                      (숫자, 영문자, 특수문자 => 1BYTE)
                                      (한글 3BYTE)
                                      (100 BYTE -> 100BYTE까지 추가하겠다. 100 CHAR -> 100글자까지 추가하겠다.)
...
*/
--> 회원관리 테이블(아이디, 비밀번호, 이름, 생년월일)을 담기위한 MEMBER 테이블
CREATE TABLE MEMBER(
...
-- USER_TABLES : 현재 사용자 계정이 가지고 있는 테이블들의 구조를 확인할 수 있는 데이터 사전

SELECT * FROM USER_TAB_COLUMNS;
-- USER_TAB_COLUMNS : 현재 사용자 계정이 가지고 있는 테이블들의 전반적인 구조를 확인할 수 있는 데이터 사전.
...
/*
    ...
    - 테이블 만들때 칼럼에 제약조건을 부여하는 방식: 컬럼레벨 방식 / 테이블레벨 방식
    
*/

/*
    1. NOT NULL 제약조건
       해당 칼럼에는 반드시 값이 존재해야만 할 경우 사용
       => NULL값이 절대 들어와서는 안되는 컬럼에 부여하는 제약조건
          데이터의 삽입/수정시 NULL값을 허용하지 않도록 제한하는 제약조건
       주의사항 : 컬럼레벨방식으로만 제약조건 부여가능. 
...
INSERT INTO MEM_NOTNULL VALUES(
    NULL, 'user01', 'pass01', '홍길동', '남', '010-1111-1111', 'aaa@naver.com'
);

INSERT INTO MEM_NOTNULL VALUES(
    2, 'user01', 'pass01', '홍길동', NULL, '010-1111-1111', 'aaa@naver.com'
);
...
CREATE TABLE MEM_UNIQUE(
    MEM_NO NUMBER NOT NULL UNIQUE, -- 컬럼레벨 방식
    MEM_ID VARCHAR2(20) NOT NULL,
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3),
    ...
    UNIQUE(MEM_ID) -- 테이블 레벨방식: 여러개의 컬럼을 따옴표로 추가할 수 있다.
    );
...
INSERT INTO MEM_UNIQUE VALUES(
    2, 'user01', 'pass01', '홍길동', '남', '010-1111-1111', 'aaa@naver.com'
); 
...
INSERT INTO MEM_UNIQUE VALUES(
    2, 'user03', 'pass01', '홍길동', '남', '010-1111-1111', 'aaa@naver.com'
); 

DROP TABLE MEM_UNIQUE;
/*
    3. CHECK 제약조건
       컬럼에 기록될 수 있는 값에 대한 조건을 설정할때 사용
       EX) 성별에는 '남'혹은 '여'만 들어오게 끔 할때 사용
       
       [표현식] 
       CHECK(조건식) 
*/

-- 성별에는 '남' 혹은 '여'만 들어오게 끔 할때 사용
...
INSERT INTO MEM_CHECK VALUES(
    1, 'user01', 'pass01', '홍길동', NULL, '010-1111-1111', 'aaa@naver.com'
); 
-- 비정상(NOT NULL 제약조건으로 안들어감) 

INSERT INTO MEM_CHECK VALUES(
    2, 'user02', 'pass01', '홍길동', '남', '010-1111-1111', 'aaa@naver.com'
); 
-- 정상 

INSERT INTO MEM_CHECK VALUES(
    3, 'user03', 'pass01', '홍길동', '국화', '010-1111-1111', 'aaa@naver.com'
);
-- 비정상(값의 크기가 안맞음)
...
/*
    DEFAULT 설정
    특정 컬럼에 들어올 값에 대한 기본값 설정(제약조건은 아니다)
    
    회원가입 컬럼에 회원정보가 삽입되는 순간 현재시간정보를 기록
...
INSERT INTO MEM_CHECK
VALUES ( 2, 'user02', 'pass01' , '홍길동', '남', NULL, NULL, DEFAULT); -- 기본값을 찾아가는것

INSERT INTO MEM_CHECK(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, GENDER) -- 원하는 값만 넣기(컬럼명을 부여해서 값을 넣는다.) 
VALUES (3, 'user03', 'pass01', '김길동', '남'); 
...
INSERT INTO MEM_PRIMARYKEY
VALUES (NULL, 'user03', 'pass01','홍길동', '남', NULL, NULL, NULL);
...
/*
    ...
    [표현법] 
    > 컬럼레벨 방식 
    컬럼명 자료형 CONSTRAINT 제약조건이름 REFERENCES 참조할테이블명(참조할컬럼명)
    
    > 테이블레벨 방식 
    CONSTRAINT 제약조건이름 FOREIGN KEY(칼럼명) REFERENCES 참조할테이블명(참조할컬럼명)
    
    주의사항 : 참조할 컬럼의 타입과 외래키로 지정할 컬럼의 타입이 같아야한다.
    
    참조할컬럼명은 생략 가능.(참조할테이블의 PK값을 기본값으로 지정하기 때문)
*/


-- 부모테이블생성
-- 회원등급에대한 데이터 보관하는 테이블
...
CREATE TABLE MEM(
    ...
    CONSTRAINT MEM_FK_GRADE FOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE /*(GRADE_CODE)*/
);
...
-- 오류 발생(G4는 MEM_GRADE에 존재하는 등급이 아니여서 에러 발생)
...
/*
    ...
    * FOREFIN KEY 삭제옵션 
    ...
    - ON DELETE SET NULL : 부모데이터를 삭제할때 해당 데이터를 참조하고 있는 자식데이터의 칼럼을 NULL로 치환.
...
CREATE TABLE MEM(
    ...
    CONSTRAINT MEM_PK_MEM_NO PRIMARY KEY(MEM_NO),
    CONSTRAINT MEM_FK_GRADE FOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE ON DELETE SET NULL/*(GRADE_CODE) ON DELETE RESTRICTED*/
);
...
CREATE TABLE MEM(
    ...
    CONSTRAINT MEM_PK_MEM_NO PRIMARY KEY(MEM_NO),
    CONSTRAINT MEM_FK_GRADE FOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE ON DELETE CASCADE/*(GRADE_CODE) ON DELETE RESTRICTED*/
);
...
-- 컬럼명, 자료형, 행데이터, NOT NULL 제약조건까지는 제대로 복사가 이루어짐. 
-- 단, PRIMARY KEY, UNIQUE, FOREGIN KEY 등은 복사가 제대로 안된다.
...
FROM EMPLOYEE
WHERE 1 = 0;
-- 결과값 거짓(모든행 데이터가 빠진 상태로 컬럼과 자료형만 남은 상태로 복사됨)
